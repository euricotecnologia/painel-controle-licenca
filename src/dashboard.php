...
//consulta no B.D.
$consulta = "SELECT * FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."'";
$mudar_senha = $con->query($consulta) or die($con->error);

//Dashboard//
$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') ";
$quant_cliente = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') ";
$quant_clienteg = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' AND status_pagamento = '1' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') AND status_pagamento = '1'";
$pag_realizado = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' AND status_pagamento = '0' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') AND status_pagamento = '0'";
$pag_nrealizado = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' AND status_pagamento = '1' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') AND status_pagamento = '1'";
$pag_realizadog = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes WHERE cnpj_revenda = '".$_SESSION['cnpj_revenda']."' AND cnpj_revenda != '' AND status_pagamento = '0' OR nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') AND status_pagamento = '0'";
$pag_nrealizadog = $con->query($consulta) or die($con->error);

$consulta = "SELECT COUNT(*) FROM clientes";
$quant_cliente2 = $con->query($consulta) or die($con->error);
//Dashboard//
...
$status_atividade[0] = "EM PROGRESSO";
$status_atividade[1] = "ATIVIDADE PAUSADA";
...
<html lang="pt-br">
	...
	<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize aside-minimize-hoverable page-loading">
							...
							<div class="container-fluid">
								...
								<div class="row">
									...
									<div class="col-xl-4">
										...
										<a class="card card-custom bg-primary bg-hover-state-dark card-stretch gutter-b">
											<!--begin::Body-->
											<div class="card-body">
												<center>
												...
												<div class="text-inverse-dark font-weight-bolder font-size-h3 mb-2 mt-5 text-center">Quantidade de Clientes</div>

												<div class="font-weight-bold text-inverse-dark font-size-h1 text-center">
													<?php while ($dado = $quant_cliente ->fetch_array()){ ?>
														<?php echo $dado['COUNT(*)'] ?>
													<?php } ?></div>
											</div>
											<!--end::Body-->
									...
									<div class="col-xl-4">
										<!--begin::Stats Widget 18-->
										<a class="card card-custom bg-success bg-hover-state-dark card-stretch gutter-b">
											<!--begin::Body-->
											<div class="card-body">
												<center>
												<span class="svg-icon svg-icon-white svg-icon-4x ml-n1">
        ...
        <path d="M15.493,8.044 C15.2143319,7.68933156 14.8501689,7.40750104 14.4005,7.1985 C13.9508311,6.98949895 13.5170021,6.885 13.099,6.885 C12.8836656,6.885 12.6651678,6.90399981 12.4435,6.942 C12.2218322,6.98000019 12.0223342,7.05283279 11.845,7.1605 C11.6676658,7.2681672 11.5188339,7.40749914 11.3985,7.5785 C11.2781661,7.74950085 11.218,7.96799867 11.218,8.234 C11.218,8.46200114 11.2654995,8.65199924 11.3605,8.804 C11.4555005,8.95600076 11.5948324,9.08899943 11.7785,9.203 C11.9621676,9.31700057 12.1806654,9.42149952 12.434,9.5165 C12.6873346,9.61150047 12.9723317,9.70966616 13.289,9.811 C13.7450023,9.96300076 14.2199975,10.1308324 14.714,10.3145 C15.2080025,10.4981676 15.6576646,10.7419985 16.063,11.046 C16.4683354,11.3500015 16.8039987,11.7268311 17.07,12.1765 C17.3360013,12.6261689 17.469,13.1866633 17.469,13.858 C17.469,14.6306705 17.3265014,15.2988305 17.0415,15.8625 C16.7564986,16.4261695 16.3733357,16.8916648 15.892,17.259 C15.4106643,17.6263352 14.8596698,17.8986658 14.239,18.076 C13.6183302,18.2533342 12.97867,18.342 12.32,18.342 C11.3573285,18.342 10.4263378,18.1741683 9.527,17.8385 C8.62766217,17.5028317 7.88033631,17.0246698 7.285,16.404 L9.413,14.238 C9.74233498,14.6433354 10.176164,14.9821653 10.7145,15.2545 C11.252836,15.5268347 11.7879973,15.663 12.32,15.663 C12.5606679,15.663 12.7949989,15.6376669 13.023,15.587 C13.2510011,15.5363331 13.4504991,15.4540006 13.6215,15.34 C13.7925009,15.2259994 13.9286662,15.0740009 14.03,14.884 C14.1313338,14.693999 14.182,14.4660013 14.182,14.2 C14.182,13.9466654 14.1186673,13.7313342 13.992,13.554 C13.8653327,13.3766658 13.6848345,13.2151674 13.4505,13.0695 C13.2161655,12.9238326 12.9248351,12.7908339 12.5765,12.6705 C12.2281649,12.5501661 11.8323355,12.420334 11.389,12.281 C10.9583312,12.141666 10.5371687,11.9770009 10.1255,11.787 C9.71383127,11.596999 9.34650161,11.3531682 9.0235,11.0555 C8.70049838,10.7578318 8.44083431,10.3968355 8.2445,9.9725 C8.04816568,9.54816454 7.95,9.03200304 7.95,8.424 C7.95,7.67666293 8.10199848,7.03700266 8.406,6.505 C8.71000152,5.97299734 9.10899753,5.53600171 9.603,5.194 C10.0970025,4.85199829 10.6543302,4.60183412 11.275,4.4435 C11.8956698,4.28516587 12.5226635,4.206 13.156,4.206 C13.9160038,4.206 14.6918294,4.34533194 15.4835,4.624 C16.2751706,4.90266806 16.9686637,5.31433061 17.564,5.859 L15.493,8.044 Z" fill="#000000"/>
												...
												<div class="text-inverse-dark font-weight-bolder font-size-h3 mb-2 mt-5 text-center">Pagamentos Realizados </div>
												<div class="font-weight-bold text-inverse-dark font-size-h1 text-center"><?php while ($dado = $pag_realizado ->fetch_array()){ ?>
														<?php echo $dado['COUNT(*)'] ?>
													<?php } ?></div>
											</div>
											<!--end::Body-->
									...
									<div class="col-xl-4">
										<!--begin::Stats Widget 18-->
										<a class="card card-custom bg-danger bg-hover-state-dark card-stretch gutter-b">
											<!--begin::Body-->
											<div class="card-body">
												<center>
												<span class="svg-icon svg-icon-white svg-icon-4x ml-n1">
												...
												<div class="text-inverse-dark font-weight-bolder font-size-h3 mb-2 mt-5 text-center">Pagamentos Pendentes</div>
												<div class="font-weight-bold text-inverse-dark font-size-h1 text-center">
													<?php while ($dado = $pag_nrealizado ->fetch_array()){ ?>
														<?php echo $dado['COUNT(*)'] ?>
													<?php } ?></div>
											</div>
											<!--end::Body-->
										...
										<div class="card card-custom gutter-b">
											...
											<div class="card-header">
												...
												<div class="card-title">
													...
													<h3 class="card-label">GRÁFICO GERAL</h3>
...
<div class="modal fade" id="tsenha" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    ...
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
        	...
        	<center><h1 class="text-danger">Altere sua senha provisória!</h1></center><br>

			<div class="form-group row">

					<div class="col-lg-12 col-xl-12">

			<form method="POST" action="../alteracao/alterar_senha.php">	

				<?php while ($dado = $mudar_senha ->fetch_array()){ ?>	

					<input type="hidden" name="id" value="<?php echo $dado['id_usuario_cli'] ?>">

					<input class="form-control form-control-solid form-control-lg" name="senha" type="password" placeholder="Informe sua nova senha" required /><?php } ?>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-lg btn-light-danger font-weight-bold" data-dismiss="modal">FECHAR</button>

                <button type="submit" class="btn btn-lg btn-danger font-weight-bold">ALTERAR</button>
	...
	var _demo2 = function () {
		...
		var options = {
			...
			xaxis: {
				...
				categories: ['Concluídos', 'Em Aberto', 'Pausados', 'Em execução', 'Total'],
	...
	var _demo3 = function () {
		...
		var options = {
			...
			series: [{
				...
				data: [<?php while ($dado = $quant_clienteg ->fetch_array()){ ?><?php echo $dado['COUNT(*)'] ?><?php } ?>,
				<?php while ($dado = $pag_realizadog ->fetch_array()){ ?><?php echo $dado['COUNT(*)'] ?><?php } ?>,
				<?php while ($dado = $pag_nrealizadog ->fetch_array()){ ?><?php echo $dado['COUNT(*)'] ?><?php } ?>]
			...
			xaxis: {
				...
				categories: ['Clientes', 'Pagamentos Realizados', 'Pagamentos Pendentes'],
