...
//consulta no B.D.
$consulta = "SELECT * FROM clientes WHERE data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND cnpj_revenda = '".$_SESSION['cnpj_revenda']."' OR data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') ORDER BY data_pagamento ASC";
$lista = $con->query($consulta) or die($con->error);

//consulta no B.D.
$consulta = "SELECT * FROM clientes WHERE data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND cnpj_revenda = '".$_SESSION['cnpj_revenda']."' OR data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') ORDER BY data_pagamento ASC";
$cliente = $con->query($consulta) or die($con->error);

//consulta no B.D.
$consulta = "SELECT * FROM clientes WHERE data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND cnpj_revenda = '".$_SESSION['cnpj_revenda']."' OR data_pagamento >= '$data_inicial' AND data_pagamento <= '$data_final' AND nivel_adm = (SELECT nivel_acesso FROM usuarios WHERE id_usuario = '".$_SESSION['id_usuario']."') ORDER BY data_pagamento ASC";
$cliente_pagamento = $con->query($consulta) or die($con->error);

//consulta no B.D.
$consulta = "SELECT * FROM sistemas";
$sistema = $con->query($consulta) or die($con->error);
...
<html lang="pt-br">
	...
	<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize aside-minimize-hoverable page-loading">
						...
						<div class="d-flex flex-column-fluid">
							...
							<div class="container-fluid">
								...
								<div class="card card-custom gutter-b">
									<!--begin::Header--><br>
									<center><h1 class="display-4 text-primary"><strong>RESULTADO DA PESQUISA POR PERÍODO</strong></h1></center>
								...
								<div class="card card-custom gutter-b">
									...
									<div class="card-header border-0 py-5">
										<h3 class="card-title align-items-start flex-column">
											<span class="card-label font-weight-bolder h2 text-primary">Período: <?php 
												$d = explode(" ", $data_inicial);
												$data = explode("-", $d[0]);
												echo "$data[2]/$data[1]/$data[0]";?> ATÉ <?php 
												$d = explode(" ", $data_final);
												$data = explode("-", $d[0]);
												echo "$data[2]/$data[1]/$data[0]";?> </span>
											<span class="text-muted mt-3 font-weight-bold font-size-h3">Contas a receber</span>
										</h3>
										<div class="card-toolbar">
											<a href="../contas.php" class="d-print-none btn btn-lg btn-warning font-weight-bolder font-size-h3"><i class="fas fa-angle-double-left icon-2x"></i>VOLTAR</a>
										</div>
									</div>
									<!--end::Header-->
									<!--begin::Body-->
												...
												<thead>
													<tr class="text-uppercase">
														<th style="min-width: 250px" class="pl-7">
															<span class="text-dark-75">EMPRESA</span>
														</th>
														<th style="min-width: 100px">VENCIMENTO</th>
														<th style="min-width: 100px">LIBERAÇÃO</th>
														<th style="min-width: 150px">VALOR</th>
														<th style="min-width: 130px">STATUS</th>
														<th style="min-width: 130px">STATUS DE PAGAMENTO</th>
														<th style="min-width: 130px">TOTAL DE LICENÇAS</th>
													</tr>
												</thead>
												<tbody>
													...
													<tr>
														<td class="pl-0 py-8">
															<div class="d-flex align-items-center">
																<div>
																	<a class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-h3"><?php echo $dado['nome_razao'] ?></a>
																	<span class="text-muted font-weight-bold font-size-h4 d-block">CNPJ: <?php echo $dado['cnpj'] ?></span>
																</div>
															</div>
														</td>
														<td>
															<span class="text-dark-75 font-weight-bolder d-block font-size-h3"><?php 
												$d = explode(" ", $dado['data_vencimento']);
												$data = explode("-", $d[0]);
												echo "$data[2]/$data[1]/$data[0]";?></span>
														</td>
														<td>
															<span class="text-dark-75 font-weight-bolder d-block font-size-h3"><?php 
												$d = explode(" ", $dado['data_limite_liberacao']);
												$data = explode("-", $d[0]);
												echo "$data[2]/$data[1]/$data[0]";?></span>
														</td>
														<td>
															<span class="text-dark-75 font-weight-bolder d-block font-size-h3">R$ <?php echo $dado['valor_entrada'] ?></span>
														</td>
														<td>
															<a style="cursor: pointer;" data-toggle="modal" data-target="#ModalStatus<?php echo $dado['id'] ?>" class="label label-xl label-light-<?php echo $st_cor[$dado['status']] ?> font-size-h3 label-inline"><?php echo $st[$dado['status']] ?></a>
														</td>
														<td>
															<a style="cursor: pointer;" data-toggle="modal" data-target="#ModalPagamento<?php echo $dado['id'] ?>" class="label label-xl label-light-<?php echo $stp_cor[$dado['status_pagamento']] ?> font-size-h3 label-inline"><?php echo $stp[$dado['status_pagamento']] ?></a>
														</td>
														<td>
															<span class="label label-xl label-light-info font-size-h3 label-inline"><?php echo $dado['limita_nota_qtde'] ?> Licenças</span>
														</td>
													</tr>
													<?php } ?>
												</tbody>
		...
		<!-- Modal Status-->
<?php while ($dado = $cliente ->fetch_array()){ ?>		
<div class="modal fade" id="ModalStatus<?php echo $dado['id'] ?>" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title text-primary" id="exampleModalLabel">Alterar Status do Sistema</h2>
            ...
            <div class="modal-body">
               <!--begin::Wizard Step 1-->
		<!--begin::Group-->
		<form method="POST" action="../editar/alterar_status_cliente.php">
				...
				<div class="col-lg-9 col-xl-9">
							...
							<label class="option">
								...
								<span class="option-label">
									<span class="option-head">
										<span class="h4 option-focus text-success">LIBERADO</span>
										</span>
									</span>
							</label>
						</div>
					...
					<div class="col-lg-6 col-sm-2">
							<label class="option">
								...
								<span class="option-label">
									<span class="option-head">
										<span class="h4 option-focus text-danger">BLOQUEADO</span>
										</span>
									</span>
							</label>
						</div>
		...
		<!--begin::Group-->
			<div class="form-group row">
				<label for="sistema" class="col-xl-3 col-lg-3 col-form-label">Justificar/Observação<strong class="text-danger">*</strong></label>
					<div class="col-lg-9 col-xl-9">
						<div class="form-group row">
							<div class="col-lg-12 col-md-12 col-sm-12">
								<input class="form-control form-control-lg" name="obs" placeholder="Justifique o motivo do bloqueio ou liberação..." type="text" required>
							</div>
						</div>
					</div>
			    </div>
    	<!--end::Group-->
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-lg btn-primary font-weight-bold">ALTERAR</button>
            </div>
            </form>
        </div>
    </div>
...
<?php while ($dado = $cliente_pagamento ->fetch_array()){ ?>		
<!-- Modal Status-->
<div class="modal fade" id="ModalPagamento<?php echo $dado['id'] ?>" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title text-primary" id="exampleModalLabel">Alterar Status de pagamento</h2>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body">
               <!--begin::Wizard Step 1-->
		<!--begin::Group-->
		<form method="POST" action="../editar/alterar_status_pagamento.php">
			<input type="hidden" name="id" value="<?php echo $dado['id'] ?>">	
		<!--begin::Group-->
		<div class="form-group row">
			<label class="col-xl-3 col-lg-3 col-form-label">Status de Pagamento <strong class="text-danger">*</strong></label>
				<div class="col-lg-9 col-xl-9">
					<div class="row">
						<div class="col-lg-6 col-sm-2">
							<label class="option">
								<span class="option-control">
									<span class="radio radio-success radio-bold radio-brand">
										<input type="radio" name="status" value="1" <?php echo $ativado[$dado['status_pagamento']] ?>>
										<span></span>
									</span>
								</span>
								<span class="option-label">
									<span class="option-head">
										<span class="h4 option-focus text-success">SIM</span>
										</span>
									</span>
							</label>
						</div>
					...
					<div class="col-lg-6 col-sm-2">
							<label class="option">
								<span class="option-control">
									<span class="radio radio-danger radio-bold radio-brand">
										<input type="radio" name="status" value="0" <?php echo $desativado[$dado['status_pagamento']] ?>>
										<span></span>
									</span>
								</span>
								<span class="option-label">
									<span class="option-head">
										<span class="h4 option-focus text-danger">NÃO</span>
										</span>
									</span>
							</label>
						</div>
